...
def login(Root_Frame, _Root_):
    ...
    Button(screen_login_frame, text="SIGN IN", bg=GlobalHelper.theme_color, fg='#fff', width='30', height='1',
           borderwidth=0, relief=SOLID, command=lambda: _Root_.show_frame("Dashboard_Manager")).grid(row=5, column=1,
                                                                                                     ipady=5, pady=10)
...
def register(Root_Frame, _Root_):
    ...
    Label(screen_login_frame, text="Sign up", bg='#ffffff', font=("Bahnschrift SemiLight Condensed", 25)).grid(row=0,
                                                                                                               column=1,
                                                                                                               pady=10,
                                                                                                               sticky=NW, )
    ...
    Label(screen_login_frame, text="Address", bg='#ffffff').grid(row=5, column=1, pady=1, sticky=NW)
    Entry(screen_login_frame, textvariable=reg_address, width=50, relief=SOLID).grid(row=6, column=1, pady=3, ipady=5)

    Label(screen_login_frame, text="Password", bg='#ffffff').grid(row=7, column=1, pady=1, sticky=NW)
    Entry(screen_login_frame, textvariable=reg_password, show='*', width=50, relief=SOLID).grid(row=8, column=1, pady=3,
                                                                                                ipady=5)

    Label(screen_login_frame, text="Confirm Password", bg='#ffffff').grid(row=9, column=1, pady=1, sticky=NW)
    Entry(screen_login_frame, textvariable=reg_confirm_password, show='*', width=50, relief=SOLID).grid(row=10,
                                                                                                        column=1,
                                                                                                        pady=3, ipady=5)

    Button(screen_login_frame, text="SIGN UP", bg=GlobalHelper.theme_color, fg='#fff', width='30', height='1',
           borderwidth=0, relief=SOLID).grid(row=11, column=1, ipady=5, pady=10)
    Button(screen_login_frame, cursor="hand2", text="Already have an account? Sign in Now", bg='#ffffff', bd=0,
           height='1', command=lambda: _Root_.show_frame("Authentication_Login")).grid(row=12, column=1)
